...
// Ayuda a paginar mejor en A4: evita que filas, imágenes, códigos y titulares
// se partan a mitad entre páginas. Los diseños pueden marcar tarjetas/bloques
// que no deben cortarse con la clase .no-break (o [data-no-break]).
const PAGE_HELPER = `
  tr, thead, img, svg, figure, .no-break, [data-no-break] { break-inside: avoid; }
...
export async function renderHtmlToPdf(html, data = {}) {
  ...
  try {
    ...
    await page.addStyleTag({ content: PAGE_HELPER });
